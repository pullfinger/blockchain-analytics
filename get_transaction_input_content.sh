...
while read detail_transaction_input_decode; do
    ...
    then
        raw_prevout=`bitcoin-cli getrawtransaction $vin_txid 1 | jq -c -f detail_transaction_output.jq | \
            ...
            jq -c '{"vin_blockhash":.blockhash ,"vin_time":.time,"vin_blocktime":.blocktime,"satoshi":.satoshi }'`
    else
        raw_prevout="{\"vin_blockhash\":null,\"vin_time\":null,\"vin_blocktime\":null,\"satoshi\":null}"
    fi
